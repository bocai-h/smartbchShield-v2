...
class ClientSuterERC20 extends ClientBase {
  constructor(web3, suter, home, erc20Token) {
    ...
    if (erc20Token === undefined) throw '4th arg should be an ERC20 contract.';
  ...
  async depositUSDT(value, approveGasLimit, fundGasLimit) {
    var that = this;
    that.checkRegistered();
    that.checkValue();
    var account = that.account;
    ...
    if (new BigNumber(allowance).minus(nativeValue) < 0) {
      if (allowance > 0) {
        ...
        await that.erc20Token.methods
          .approve(that.suter.options.address, 0)
          .send({ from: that.home, gas: approveGasLimit });
      }
      await that.erc20Token.methods
        .approve(that.suter.options.address, nativeValue)
